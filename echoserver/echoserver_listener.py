...
from echoserver_connection import EchoServerConnection, EchoServerConnectionDisconnectException
...
class EchoServerListener():
    ...
    def listen(self):
            ...
            while self._input:
                ...
                except EchoServerConnectionDisconnectException as e:
                    logger.error("Connection disconnected: %s" % (e.message,))
                    connection = e.echoserver_connection.getConnection()
                    logger.debug("Connection %s" % (connection,))
                    
                    e.echoserver_connection.close()
                    if connection in self._output:  self._output.remove(connection)
                    if connection in self._input:   self._input.remove(connection)
                    if connection in self._connections: del self._connections[connection]
